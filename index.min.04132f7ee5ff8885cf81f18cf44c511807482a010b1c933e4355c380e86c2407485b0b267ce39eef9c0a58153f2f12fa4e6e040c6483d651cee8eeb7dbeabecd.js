...
Don\u0026rsquo;t forget to change the image url so it corresponds to the one used when building the package.
apiVersion: package-operator.run/v1alpha1 kind: Package metadata: name: my-nginx spec: image: \u0026lt;your-image-url-goes-here\u0026gt; $ kubectl create -f 1_applications/package.yaml package.package-operator.run/my-nginx created # wait for package to be loaded and installed: $ kubectl get package -w NAME STATUS AGE my-nginx Progressing 11s my-nginx Available 13s # success! $ kubectl get po NAME READY STATUS RESTARTS AGE nginx-deployment-cd55c47f5-szvh7 1/1 Running 0 79s 2. Templates # Please refer to the files in /1_applications/2_templates for this step.
...
TemplateContext is documented as part of the API.
app.kubernetes.io/instance: \u0026quot;{{.Package.Name}}\u0026quot; Testing Templates # Using a template engine with yaml files can quickly lead to unexpected results.
...
`}),e.add({id:10,href:"/docs/concepts/probes/",title:"Probes",description:`Probes define how Package Operator judges the Availability of objects and is reporting status.
...
`}),e.add({id:13,href:"/docs/getting_started/api-reference/",title:"API Reference",description:`The Package Operator APIs are an extension of the Kubernetes API using CustomResourceDefinitions. These new APIs can be interacted with like any other Kubernetes object using e.g. kubectl.
...
Example
apiVersion: package-operator.run/v1alpha1 kind: ObjectSlice metadata: name: example namespace: default objects: - object: apiVersion: apps/v1 kind: Deployment metadata: name: example-deployment Field Description metadata metav1.ObjectMeta objects required
[]ObjectSetObject Package # Example
...
Field Description conditions []metav1.Condition Conditions is a list of status conditions the object is in. phase ObjectDeploymentPhase This field is not part of any API contract
...
Field Description conditions []metav1.Condition Conditions is a list of status conditions the object is in. controllerOf []ControlledObjectReference References all objects controlled by this instance. Used in:
...
Field Description conditions []metav1.Condition Conditions is a list of status conditions the object is in. phase ObjectSetStatusPhase Phase is not part of any API contract
it will go away as soon as kubectl can print conditions!
...
Field Description conditions []metav1.Condition Conditions is a list of status conditions the object is in. phase PackageStatusPhase This field is not part of any API contract
it will go away as soon as kubectl can print conditions!
...
All probes need to succeed for a package to be considered Available.
Failing probes will prevent the reconciliation of objects in later phases. Used in:
PackageManifest PackageManifestTest # PackageManifestTest configures test cases.
...
TemplateContextPackage TemplateContextPackage represents the (Cluster)Package object requesting this package content. Used in:
...
Field Description name required
string namespace string labels map[string]string annotations map[string]string Used in:
...
apiVersion: package-operator.run/v1alpha1 kind: ObjectDeployment spec: template: spec: phases: - name: rbac-and-stuff class: hosted-cluster objects: [] - name: deploy objects: [] - name: configure class: hosted-cluster objects: [] `}),e.add({id:18,href:"/docs/",title:"Docs",description:"Docs Doks.",content:""}),search.addEventListener("input",t,!0);function t(){const s=5;var n=this.value,o=e.search(n,{limit:s,enrich:!0});const t=new Map;for(const e of o.flatMap(e=>e.result)){if(t.has(e.doc.href))continue;t.set(e.doc.href,e.doc)}if(suggestions.innerHTML="",suggestions.classList.remove("d-none"),t.size===0&&n){const e=document.createElement("div");e.innerHTML=`No results for "<strong>${n}</strong>"`,e.classList.add("suggestion__no-results"),suggestions.appendChild(e);return}for(const[r,a]of t){const n=document.createElement("div");suggestions.appendChild(n);const e=document.createElement("a");e.href=r,n.appendChild(e);const o=document.createElement("span");o.textContent=a.title,o.classList.add("suggestion__title"),e.appendChild(o);const i=document.createElement("span");if(i.textContent=a.description,i.classList.add("suggestion__description"),e.appendChild(i),suggestions.appendChild(n),suggestions.childElementCount==s)break}}})()